    ...
    <form class="p-5 border" @submit.prevent="register">
      ...
      <div class="form-group">
        <label for="password">
          パスワード
          <br />
          <span class="error_message">{{checkPassword}}</span>
        </label>
      ...
      <div class="form-group">
        <label for="password_confirmation">
          確認用パスワード
          <br />
          <span class="error_message">{{checkSamePassword}}</span>
        </label>
        <input
          type="password"
          class="form-control"
      ...
      <div v-if="validate">
        <button class="btn btn-info form-control">登録する</button>
      </div>
      <div v-else class="text-center p-3 dummy">
        <span class="btn btn-info form-control">登録する</span>
      </div>
    </form>
  </div>
</template>
